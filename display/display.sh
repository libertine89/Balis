...
function display() {
    ...
    #### Arrays in display.conf
    #IFS= ':' read -r INSTALL_DRIVER_PACKAGES PACKAGES_DRIVER PACKAGES_DRIVER_MULTILIB <<< "${DISPLAY_DRIVER_DEPENDENCIES[$DISPLAY_DRIVER]}"
    IFS=':' read -r -a driver_parts <<< "${DISPLAY_DRIVER_DEPENDENCIES[$DISPLAY_DRIVER]}"
        INSTALL_DRIVER_PACKAGES="${driver_parts[0]}"
        PACKAGES_DRIVER="${driver_parts[1]}"
        PACKAGES_DRIVER_MULTILIB="${driver_parts[2]}"
    ...
    if [ "$VULKAN" == "true" ]; then
        IFS= ':' read -r PACKAGES_VULKAN PACKAGES_VULKAN_MULTILIB <<< "${VULKAN_DEPENDENCIES[$DISPLAY_DRIVER]}"
    fi
    ### --- ###
    ...
    if [[ 
        ...
        IFS= ':' read -r PACKAGES_HARDWARE_ACCELERATION PACKAGES_HARDWARE_ACCELERATION_MULTILIB <<< "${HARDWARE_ACCELERATION_DEPENDENCIES[$DISPLAY_DRIVER]}"
    fi
    ### --- ###
